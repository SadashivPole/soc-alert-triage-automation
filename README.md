...
| **TheHive 5 CE** | Case-management export |  implemented + `thehive` compose profile; fake-transport/tests only, **no live CE run claimed** (optional; CE only, never Premium) | Free, self-hosted |
...
- **TheHive CE case export** (optional) is **implemented** — client, export builder, API,
  tests, and the optional `thehive` compose profile (CE only, never Premium). It is
  disabled by default (empty `THEHIVE_URL`/`THEHIVE_API_KEY`) and validated by tests
