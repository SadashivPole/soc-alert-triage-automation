...
* Workflow JSON logic: reads $env secret, exact match, rejects missing/wrong, never logs token
...
def test_n8n_auth_missing_token_rejected() -> None:
    ...
    with pytest.raises(HTTPException) as exc:
        verify_n8n_token(req, x_n8n_token=None, x_callback_token=None, authorization=None)
...
def test_n8n_auth_wrong_token_rejected() -> None:
    ...
    req = _make_request(settings)
    with pytest.raises(HTTPException) as exc:
        verify_n8n_token(req, x_n8n_token="wrong-token", x_callback_token=None, authorization=None)
    assert exc.value.status_code == 401
    assert "invalid" in exc.value.detail.lower()

    with pytest.raises(HTTPException) as exc2:
        verify_n8n_token(req, x_n8n_token=None, x_callback_token=None, authorization="Bearer wrong")
...
def test_n8n_auth_correct_token_accepted() -> None:
    ...
    # X-N8N-Token header
    verify_n8n_token(
        req, x_n8n_token="expected-secret-token", x_callback_token=None, authorization=None
    )

    # X-Callback-Token header
    verify_n8n_token(
        req, x_n8n_token=None, x_callback_token="expected-secret-token", authorization=None
    )

    # Bearer
    verify_n8n_token(
        req, x_n8n_token=None, x_callback_token=None, authorization="Bearer expected-secret-token"
    )
...
def test_wf1_workflow_json_real_validation() -> None:
    ...
    validate_nodes = [n for n in nodes if "validate" in n.get("name", "").lower()]
    assert validate_nodes, "No validate node found in WF1"

    code = ""
    for n in validate_nodes:
        params = n.get("parameters", {})
        if "functionCode" in params:
            code += params["functionCode"]

    # Must read from $env.N8N_CALLBACK_TOKEN or $env.N8N_WEBHOOK_TOKEN (env credential reference)
    ...
    assert "!==" in code or "!=" in code or "compare" in code.lower(), "WF1 must do comparison"
    ...
    # Never log token (no console.log of token, no logging token variable)
    lower_code = code.lower()
    # Ensure we don't log supplied/expected token values
    # Allowed: console.log for contain_requested message, but not token
    assert "console.log(supplied" not in lower_code
    assert "console.log(expected" not in lower_code
    # Ensure no secret hardcoded in JSON (outside env ref)
    ...
    # Notes must mention never log token / env credential
    notes_combined = " ".join([n.get("notes", "") for n in nodes]).lower()
    assert "env" in notes_combined or "$env" in dumped, "WF1 notes should mention env credential"


def test_wf1_no_secret_in_json() -> None:
    data = json.loads(WF1_PATH.read_text())
    # Ensure no hardcoded token values (the JSON itself should not contain real secret)
    # Only credential references allowed
    # Check that header value uses $env reference, not literal secret
        ...
        if node.get("type") == "n8n-nodes-base.httpRequest":
            ...
            for h in headers:
                if "token" in h.get("name", "").lower():
                    ...
                    assert "$env" in val or "{{" in val, (
                        f"Token header must use env reference, got {val}"
                    )
                    # Must not be a literal secret like "secret123"
                    assert len(val) < 100  # env refs are short
    ...
    for wf_path in workflow_dir.glob("*.json"):
        ...
        for node in data.get("nodes", []):
            ...
            for hp in header_params:
                if "token" in hp.get("name", "").lower():
                    val = hp.get("value", "")
                    assert "$env" in val, (
                        f"{wf_path.name} node {node.get('name')} must use $env token reference, got {val}"
                    )
