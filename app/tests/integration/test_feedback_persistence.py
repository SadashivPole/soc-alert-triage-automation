...
the application factory's normal startup path twice over the same SQLite file,
so it is deterministic on Windows and Linux (no ``chmod`` semantics).
"""
...
def test_status_before_any_feedback_reports_unacknowledged(
    db_url: str,
) -> None:
...
def test_true_positive_feedback_submission_updates_status(
    db_url: str,
) -> None:
...
def test_feedback_persists_across_application_restart(
    db_url: str,
) -> None:
...
def test_feedback_received_audit_entry_survives_restart(
    db_url: str,
) -> None:
...
def test_feedback_endpoints_reject_missing_or_invalid_token(
    db_url: str,
) -> None:
...
def test_escalate_verdict_accepted_and_persisted_across_restart(
    db_url: str,
) -> None:
...
def test_multiple_feedback_entries_count_and_keep_latest_after_restart(
    ...
    assert [row.verdict for row in stored] == [
        "true_positive",
        "escalate",
    ]
