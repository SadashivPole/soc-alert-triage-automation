...
| **Phase 4 - Real Wazuh integration & approved response** | partially validated | 4.1/4.2 source-audited + live-validated end-to-end; 4.4 validated; V7/V8 runtime-verified; V9 remains partially live-validated with the open_incident-specific duplicate check outstanding; 4.3 custom rules/decoders live-validated; 4.5 containment approval-request flow runtime-validated; 4.6 TheHive CE live-validated for D1 note propagation and repeated-export idempotency; the D2 recovery lookup endpoint and TheHive client path are live-validated, while the complete orphan-recovery export route remains regression-tested only; 4.7 soak completed operator-side with 10,000/10,000 persistence, five transport timeouts later confirmed persisted, and clean 25/25 and 500/500 follow-up runs |
...
| 4.6 | Optional TheHive CE case export (CE only) | live-validated 2026-09-21 for D1 note propagation and repeated-export idempotency | `app/src/soc_triage/thehive/{client,export}.py` + `api/incident_thehive.py` + settings + optional additive `thehive` compose profile. Live evidence: fresh controlled incident `INC-2026-09-21-0001`, note propagation, first export created `~4320`, second export returned the same `~4320` with `duplicate=true`, exactly one `incident.thehive_exported` row, and exactly one matching TheHive case. D2 recovery lookup endpoint and TheHive client path are live-validated; the complete orphan-recovery export route remains regression-tested only. Premium never required. |
| 4.7 | Performance soak: 10k synthetic alerts/day | completed operator-side runtime validation | `scripts/phase47_soak.py` executed sequentially against the live ingest API: 10,000/10,000 alerts persisted; five client transport timeouts occurred during the long run and all five corresponding alerts were later confirmed persisted. Follow-up runs: 25/25 HTTP 202 with 0 errors and 500/500 HTTP 202 with 500/500 database persistence. The run produced latency/throughput evidence but is not a claim of SLA-safe production performance. |
...
| V8 post-Phase-4 `/metrics` hygiene (no new families, no Wazuh data) | ✅ runtime-verified — live `/metrics` scrape on 2026-09-20 exposed only the approved application metric families and no Wazuh-related metric data. |
...
| 4.6 TheHive CE export **live** run against a started CE instance | ? verified 2026-09-21 (D1 note propagation + D2 idempotency/recovery; exact-one-case and single-audit-row checks passed) |
| 4.7 10k alerts/day soak execution | verified 2026-09-24 | 10,000/10,000 alerts persisted; 5 client transport timeouts were observed and all 5 corresponding alerts were later confirmed persisted; 25/25 and 500/500 follow-up runs returned HTTP 202, with 500/500 confirmed persisted |
