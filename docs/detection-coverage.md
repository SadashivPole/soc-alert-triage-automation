...
`test_evaluation.py`. Phase 6.3 grew the corpus from ten to sixteen scenarios and then
to twenty-four, adding per-scenario negatives, custom-rule near-misses, and a
just-below-trigger recurrence boundary (`SCN-17`–`SCN-24`).
...
| `SCN-17` | `17_wazuh_fim_authorized_motd.json` | `550` (level 7) | `T1566` | 43 | low | `monitor` | `docs/runbooks/fim-critical-file.md` | `test_evaluation.py::test_ground_truth_evaluation`, `test_scoring_pipeline.py::test_authorized_fim_modification_stays_monitor` | **Locally validated; authorized FIM negative** |
| `SCN-18` | `18_wazuh_windows_user_created_expected.json` | `60180` (level 5) | `T1136` | 32 | low | `monitor` | `docs/runbooks/account-creation.md` | `test_evaluation.py::test_ground_truth_evaluation`, `test_scoring_pipeline.py::test_expected_account_creation_stays_monitor` | **Locally validated; expected-account negative** |
| `SCN-19` | `19_wazuh_virustotal_no_match.json` | `87103` (level 3) | — (none declared) | 25 | low | `monitor` | `docs/runbooks/malware-hash.md` | `test_evaluation.py::test_ground_truth_evaluation`, `test_scoring_pipeline.py::test_non_malicious_hash_event_stays_monitor` | **Locally validated; non-malicious hash negative** |
| `SCN-20` | `20_custom_ssh_near_miss.json` | `5712` (level 5) | `T1110` | 43 | low | `monitor` | `docs/runbooks/ssh-brute-force.md` | `test_evaluation.py::test_ground_truth_evaluation`, `test_scoring_pipeline.py::test_custom_ssh_near_miss_stays_monitor` | **Locally validated; custom SSH near-miss** |
| `SCN-21` | `21_custom_fim_near_miss.json` | `553` (level 7) | — (none declared) | 38 | low | `monitor` | `docs/runbooks/fim-critical-file.md` | `test_evaluation.py::test_ground_truth_evaluation`, `test_scoring_pipeline.py::test_custom_fim_near_miss_stays_monitor` | **Locally validated; custom FIM near-miss** |
| `SCN-22` | `22_custom_web_near_miss.json` | `31100` (level 3) | — (none declared) | 27 | low | `monitor` | `docs/runbooks/web-attack.md` | `test_evaluation.py::test_ground_truth_evaluation`, `test_scoring_pipeline.py::test_custom_web_near_miss_stays_monitor` | **Locally validated; custom web near-miss** |
| `SCN-23` | `23_suspicious_web_non_triggering.json` | `31101` (level 3) | — (none declared) | 27 | low | `monitor` | `docs/runbooks/web-attack.md` | `test_evaluation.py::test_ground_truth_evaluation`, `test_scoring_pipeline.py::test_suspicious_non_triggering_web_request_stays_monitor` | **Locally validated; non-triggering web negative** |
| `SCN-24` | `24_ssh_recurrence_below_burst.json` | `5710` (level 5) | `T1110` | 43 | low | `monitor` | `docs/runbooks/ssh-brute-force.md` | `test_evaluation.py::test_ground_truth_evaluation`, `test_scoring_pipeline.py::test_recurrence_boundary_two_deliveries_do_not_escalate` | **Locally validated; recurrence-boundary negative** |
...
| G8 | **Corpus quality and detection-quality metrics are gated in CI; live Wazuh coverage is still not measured** — Phase 6.6 uses `app/tests/evaluation/detection_quality.py` and `app/tests/evaluation/test_evaluation.py` to build a `ConfusionMatrix` over the 24-scenario corpus and assert `DetectionQualityThresholds`: precision ≥ 1.0, recall ≥ 11/12, F1 ≥ 22/23, FPR ≤ 0.0, TP ≥ 11, FP ≤ 0, FN ≤ 1, TN ≥ 12. The current measured corpus result is TP 11 / FP 0 / FN 1 / TN 12, precision 1.0, recall 0.9167, F1 0.9565, FPR 0.0. `app/tests/evaluation/test_detection_quality_gates.py` additionally validates corpus completeness and proves the gate rejects a degraded metric. The gate runs automatically through the existing `python-checks` pytest job; no new workflow was added. This is a **corpus/detection-quality** gate, not a claim of live Wazuh rule coverage, rule health, or manager-side firing, and it does not assert a percentage live-Wazuh coverage metric. | `app/tests/evaluation/detection_quality.py`; `app/tests/evaluation/test_evaluation.py`; `app/tests/evaluation/test_detection_quality_gates.py` | Implemented (Phase 6.6) — corpus/detection-quality CI gate; not live Wazuh coverage |
...
| G10 | **Allowlist suppression is not pinned end-to-end by the corpus** — the static `allowlist.v1` provider/loader now exists (Phase 2.2), loads only when `TRIAGE_ALLOWLIST_PATH` is configured, matches exact normalized indicators and IPv4 CIDR, reports `enrichment_status: skipped`, and feeds the existing `allowlist` modifier (−20) plus the `suppress` decision route. The remaining gap is that the 24-scenario corpus does not pin a `suppress` outcome; the composition is covered by unit/integration wiring rather than an end-to-end corpus case | `app/config/allowlists.yaml`; `app/src/soc_triage/enrichment/allowlist.py`; `app/config/scoring.yaml` (`allowlist`); `app/config/decisions.yaml` (`allowlisted_action`) | Outstanding — corpus-level `suppress` pin |
