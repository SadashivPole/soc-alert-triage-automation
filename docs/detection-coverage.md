...
| G8 | **Coverage is documented, not measured** — no automated coverage metric, no threshold, and no CI gate. The framework validates *traceability*, not detection quality | this document + `app/tests/evaluation/test_detection_coverage.py` | Outstanding (roadmap item 6.6 — Phase 6.2's ATT&CK mapping registry adds mapping *traceability*, not quality measurement) |
